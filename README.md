...
1. Download the latest `CraftHeads-*.jar` from releases
2. Place the JAR in your server's `plugins/` folder
3. Start the server to generate the default configuration
4. Configure head definitions in `plugins/CraftHeads/config.yml`
5. Restart the server or use `/craftheads reload`

## Commands

| Command | Description | Permission |
|---------|-------------|------------|
| `/craftheads show` | Open the head catalog menu | `craftheads.catalog` |
| `/craftheads search <query>` | Search for heads by name or tag | `craftheads.catalog` |
| `/craftheads reload` | Reload configuration and head definitions | `craftheads.admin` |
| `/craftheads give <head_id> [player] [amount]` | Give a head to a player | `craftheads.admin` |
...
See the `craftheads/src/main/resources/heads/` directory for examples.
